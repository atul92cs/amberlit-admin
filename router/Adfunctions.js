...
router.get('/',(req,res)=>{
Ad.sequelize.query('select ads.id,ads.Title,ads.Content,ads.Picture,ads.Status,subcategories.Name as Subcategory,categories.Name as Category,users.Name as User from ads join categories on ads.Category=categories.id join subcategories on ads.Subcategory=subcategories.id join users on ads.Postedby=users.id',{type:Ad.sequelize.QueryTypes.SELECT}).then(result=>{
  res.status(200).json({
    result
  });
}).catch(err=>{
